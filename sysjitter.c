...
#  define relax()          YieldProcessor()
static void frc(uint64_t* pval)
{
...
static cycles_t __measure_cpu_hz(void)
{
  ...
  sec = ElapsedMicroseconds.QuadPart / 1e6;
...
static double cycles_to_sec_f(const struct thread* t, uint64_t cycles)
{
...
static VOID CALLBACK handle_alarm(
...
static int thread_main(void* arg)
{
  ...
  TEST(SetThreadAffinityMask(t->thread_h, 1LL << t->core_i) != 0);
  ...
  if( atomic_inc(&g.n_threads_ready) == g.n_threads )
    TEST(CreateTimerQueueTimer(&g_hTimer,NULL,handle_alarm,NULL,g.runtime_secs*1000,0,WT_EXECUTEDEFAULT) == 1);
...
{
  int i, n = (int)(t->c_interruption - t->interruptions);
  for( i = 0; i < n; ++i )
...
{
  int n = (int)(t->c_interruption - t->interruptions);
...
static void thread_calc_stats(struct thread* t)
{
  ...
  t->int_n = (int)(t->c_interruption - t->interruptions);
  if( t->int_n ) {
...
static void run_expt(struct thread** threads, int runtime_secs)
{
	...
	for (i = 0; i < g.n_threads; ++i)
	{
		threads[i]->thread_h = (HANDLE)_beginthreadex(NULL,0,thread_main,threads[i],0,NULL);
	}
  ...
  for( i = 0; i < g.n_threads; ++i )
    WaitForSingleObject(threads[i]->thread_h,INFINITE);
...
int main(int argc, char* argv[])
{
  ...
  char dummy;
  unsigned int i, runtime = 70;
  SYSTEM_INFO sysinfo;
  GetSystemInfo(&sysinfo);

  g.max_interruptions = 1000000;
  g.n_threads = sysinfo.dwNumberOfProcessors;
